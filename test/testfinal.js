...
function lexer(input) {
  ...
  while (cursor < input.length) {
    ...
    // Multi-word keywords pehle check karo — warna "nahi" akela
    // identifier ban jayega aur "nahi to" detect nahi hoga
...
// ─────────────────────────────────────────────────────────────
//  STEP 2: PARSER
//  Tokens ki list ko AST (tree) mein badalta hai.
//  Tree mein har node ek kaam represent karta hai.
// ─────────────────────────────────────────────────────────────
function parser(tokens) {
  ...
  // ─── Expression parsing ───────────────────────────────────
  // Order of operations — pehle comparison, phir +/-, phir */
  // Yeh hierarchy ensure karti hai ke 2 + 3 * 4 = 14 ho, 20 nahi
...
//  AST (tree) ko wapas JavaScript code mein badalta hai.
//  Phir uss JS ko eval() se chalate hain.
// ─────────────────────────────────────────────────────────────
function codegen(node) {
  switch (node.type) {
    ...
    // "je (din) { nahi je 1: ... }" → "switch(din) { case 1: ... }"
...
// expressions ke liye alag function — value return karta hai, statement nahi
...
// ─────────────────────────────────────────────────────────────
//  STEP 4: RUNNER
//  Sab kuch ek saath — lexer → parser → codegen → eval
// ─────────────────────────────────────────────────────────────
function runner(input) {
  ...
  // optional: debugging ke liye yeh uncomment karo
  // console.log(JSON.stringify(ast, null, 2));
  // console.log(jsCode);

  eval(jsCode);                        // Step 4: chalao!
}
...
// Example 1 — Tax calculator using ya to / nahi to / nahi fer
const taxCalc = `
ye kamai = 120000
...
// Example 2 — Day of week using je / nahi je / mukhna
const dayCheck = `
ye din = 3
